-- Enhanced reports table with report source classification and 10-digit keys
CREATE TABLE IF NOT EXISTS reports (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    case_id VARCHAR(50) UNIQUE NOT NULL, -- 10-digit alphanumeric key
    case_number VARCHAR(50) UNIQUE NOT NULL,
    title VARCHAR(255) NOT NULL,
    description TEXT,
    category VARCHAR(50) CHECK (category IN ('fraud', 'abuse', 'discrimination', 'harassment', 'safety', 'corruption')),
    status VARCHAR(50) DEFAULT 'open' CHECK (status IN ('open', 'under_investigation', 'resolved', 'escalated', 'closed')),
    priority VARCHAR(20) DEFAULT 'medium' CHECK (priority IN ('low', 'medium', 'high', 'critical')),
    
    -- Report Source Classification
    report_source VARCHAR(20) DEFAULT 'ManualReport' CHECK (report_source IN ('VAPIReport', 'MapReport', 'ManualReport')),
    report_type VARCHAR(50) DEFAULT 'written',
    
    -- Location data for map reports
    location VARCHAR(500),
    latitude DECIMAL(10, 8),
    longitude DECIMAL(11, 8),
    
    -- Contact and anonymity
    is_anonymous BOOLEAN DEFAULT true,
    contact_info TEXT,
    date_occurred DATE,
    
    -- Tracking and security
    secret_code VARCHAR(100) NOT NULL,
    report_id VARCHAR(10) UNIQUE NOT NULL, -- 10-digit alphanumeric
    tracking_code VARCHAR(100),
    
    -- VAPI specific fields
    vapi_report_summary TEXT,
    vapi_session_id VARCHAR(255),
    vapi_transcript TEXT,
    vapi_audio_url TEXT,
    vapi_call_data JSONB,
    
    -- Reward system
    reward_amount DECIMAL(10,2) DEFAULT 0,
    recovery_amount DECIMAL(10,2) DEFAULT 0,
    reward_status VARCHAR(20) DEFAULT 'pending' CHECK (reward_status IN ('pending', 'approved', 'paid')),
    crypto_address VARCHAR(255),
    crypto_currency VARCHAR(20),
    
    -- Case management
    assigned_to VARCHAR(255),
    assigned_by VARCHAR(255),
    resolution_summary TEXT,
    whistleblower_update TEXT,
    
    -- Structured data for AI processing
    structured_data JSONB,
    
    -- Timestamps
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_reports_case_id ON reports(case_id);
CREATE INDEX IF NOT EXISTS idx_reports_report_source ON reports(report_source);
CREATE INDEX IF NOT EXISTS idx_reports_status ON reports(status);
CREATE INDEX IF NOT EXISTS idx_reports_category ON reports(category);
CREATE INDEX IF NOT EXISTS idx_reports_created_at ON reports(created_at);
CREATE INDEX IF NOT EXISTS idx_reports_location ON reports(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;

-- Update existing cases table to match new structure if needed
ALTER TABLE cases ADD COLUMN IF NOT EXISTS report_source VARCHAR(20) DEFAULT 'ManualReport';
ALTER TABLE cases ADD COLUMN IF NOT EXISTS latitude DECIMAL(10, 8);
ALTER TABLE cases ADD COLUMN IF NOT EXISTS longitude DECIMAL(11, 8);
ALTER TABLE cases ADD COLUMN IF NOT EXISTS location VARCHAR(500);
ALTER TABLE cases ADD COLUMN IF NOT EXISTS is_anonymous BOOLEAN DEFAULT true;
ALTER TABLE cases ADD COLUMN IF NOT EXISTS contact_info TEXT;
ALTER TABLE cases ADD COLUMN IF NOT EXISTS date_occurred DATE;
ALTER TABLE cases ADD COLUMN IF NOT EXISTS vapi_call_data JSONB;

-- Create VAPI reports table for staging
CREATE TABLE IF NOT EXISTS vapi_reports (
    id VARCHAR(255) PRIMARY KEY,
    report_id VARCHAR(10) UNIQUE NOT NULL, -- 10-digit alphanumeric
    summary TEXT,
    transcript TEXT,
    audio_url TEXT,
    session_id VARCHAR(255),
    status VARCHAR(50) DEFAULT 'pending',
    vapi_call_data JSONB,
    processed_to_case_id VARCHAR(50), -- Reference to reports.case_id
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    processed_at TIMESTAMP WITH TIME ZONE
);

-- Create index for VAPI reports
CREATE INDEX IF NOT EXISTS idx_vapi_reports_status ON vapi_reports(status);
CREATE INDEX IF NOT EXISTS idx_vapi_reports_processed ON vapi_reports(processed_to_case_id);
