...
export default function EthicsOfficerDashboard() {
  ...
  const [vapiConnectionStatus, setVapiConnectionStatus] = useState<{
    connected: boolean
    error?: string
    lastTested?: string
  }>({ connected: false })
  ...
  useEffect(() => {
    ...
    // Auto-refresh VAPI reports every 60 seconds (reduced frequency)
    const interval = setInterval(() => {
      if (vapiConnectionStatus.connected) {
        fetchVAPIReports()
      }
    }, 60000)
  ...
  const testVAPIConnection = async () => {
    try {
      ...
      const result = await vapiClient.testConnection()

      setVapiConnectionStatus({
      ...
      if (result.success) {
        ...
        console.error("❌ VAPI connection failed:", result.error)
      }
    } catch (error) {
  ...
  const fetchVAPIReports = async () => {
    if (!vapiConnectionStatus.connected) {
      console.log("Skipping VAPI fetch - not connected")
      return
    }
    ...
    try {
      ...
      setVapiReports(reports)

      // Process new VAPI reports and save them to the database
      await processVAPIReportsIntoDB(reports)
    } catch (error) {
      console.error("Error fetching VAPI reports:", error)
      setVapiReports([])

      // Update connection status if we get auth errors
  ...
  return (
    <DashboardLayout role="ethics-officer">
      <div className="space-y-6">
        ...
        {!vapiConnectionStatus.connected && (
          <div className="bg-red-900/20 border border-red-700 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <WifiOff className="h-5 w-5 text-red-400 mr-2" />
                <div>
                  <span className="text-red-300">VAPI Connection Failed</span>
                  {vapiConnectionStatus.error && (
                    <p className="text-red-400 text-sm mt-1">{vapiConnectionStatus.error}</p>
                  )}
                </div>
              </div>
              <Button
        ...
        <Tabs defaultValue="reports" className="space-y-6">
          <TabsList className="bg-slate-800/50 border-slate-700">
            ...
            >
              ...
              {!vapiConnectionStatus.connected && <WifiOff className="h-4 w-4 ml-1 text-red-400" />}
            </TabsTrigger>
          </TabsList>
              ...
              <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <CardDescription className="text-slate-400">
                      Raw reports from VAPI voice assistant
                      {vapiConnectionStatus.lastTested && (
                        <span className="block text-xs mt-1">
                          Last tested: {new Date(vapiConnectionStatus.lastTested).toLocaleString()}
                        </span>
                      )}
                    </CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      disabled={loadingVAPI || !vapiConnectionStatus.connected}
                    ...
                    >
                      {vapiConnectionStatus.connected ? "✅ Connected" : "❌ Disconnected"}
                    </Badge>
                    <Badge variant="outline" className="border-blue-500 text-blue-400">
              ...
              <CardContent>
                {!vapiConnectionStatus.connected ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-400">VAPI Connection Failed</p>
                    <p className="text-slate-500 text-sm mb-4">
                      {vapiConnectionStatus.error || "Unable to connect to VAPI service"}
                    </p>
                    <Button
                      onClick={testVAPIConnection}
                      variant="outline"
