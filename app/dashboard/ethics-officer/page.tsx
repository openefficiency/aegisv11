  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        const finalCases = casesData && casesData.length > 0 ? casesData : mockCases
  ...
  const fetchVAPIReports = async () => {
    ...
    try {
      ...
      const hasValidConfig =
        ...
        process.env.NEXT_PUBLIC_VAPI_API_KEY !== "2ca2e718-80b2-454a-a78b-e0560a06f1c4"
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="rewards" className="space-y-6">
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
              ...
              <CardContent>
                {cases.filter((c) => c.status === "resolved" && c.reward_status === "approved").length === 0 ? (
                  <div className="text-center py-12">
                    <Award className="h-12 w-12 text-slate-500 mx-auto mb-4" />
                    <p className="text-slate-400">No rewards ready for processing</p>
                    <p className="text-slate-500 text-sm">Resolved cases with approved rewards will appear here</p>
                  </div>
                ) : (
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {cases
                        .filter((c) => c.status === "resolved" && c.reward_status === "approved")
                        .map((case_) => (
                          <TableRow key={case_.id} className="border-slate-700">
                            <TableCell className="text-slate-300 font-mono">
                              {case_.tracking_code || case_.report_id || case_.case_number}
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-white">{selectedCase.structured_data.incident?.type || "N/A"}</p>
                      </div>
                      <div>
                        <Label className="text-slate-300">Incident Date</Label>
                        <p className="text-white">{selectedCase.structured_data.incident?.date || "N/A"}</p>
                      </div>
                    </div>
                    <div>
                      <Label className="text-slate-300">Follow Up Notes</Label>
                      <div className="bg-slate-900/50 p-3 rounded border border-slate-600">
                        <p className="text-slate-300">{selectedCase.structured_data.follow_up?.notes || "N/A"}</p>
                      </div>
                    </div>
                  </>
                )}
        ...
        {selectedCase && actionType === "assign" && (
          ...
          >
            <DialogContent className="bg-slate-800 border-slate-700">
              <DialogHeader>
                <DialogTitle className="text-white">Assign Case to Investigator</DialogTitle>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select onValueChange={(value) => handleAssignCase(selectedCase.id, value)}>
          ...
          >
            <DialogContent className="bg-slate-800 border-slate-700 max-w-2xl">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-slate-300">Reward Amount (Optional)</Label>
                    <Input
                      type="number"
          ...
          >
            <DialogContent className="bg-slate-800 border-slate-700">
              <DialogHeader>
                ...
                <DialogDescription className="text-slate-400">
                  Escalate case {selectedCase.tracking_code} to legal/senior management
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div>
