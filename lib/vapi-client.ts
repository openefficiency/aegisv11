...
export class VAPIClient {
  ...
  async fetchCalls(limit = 100): Promise<VAPICall[]> {
    try {
      console.log("Fetching calls from VAPI API with credentials...")
...
export const vapiClient = new VAPIClient({
  ...
  shareKey: "6a029118-46e8-4cda-87f3-0ac2f287af8f", // Using API key as share key
})
...
export const testVAPICredentials = async () => {
  try {
    ...
    const response = await fetch(
      `https://api.vapi.ai/assistant/${process.env.NEXT_PUBLIC_VAPI_ASSISTANT_ID || "265d793f-8179-4d20-a6cc-eb337577c512"}`,
      {
        headers: {
          Authorization: `Bearer ${process.env.NEXT_PUBLIC_VAPI_API_KEY || "6a029118-46e8-4cda-87f3-0ac2f287af8f"}`,
          "Content-Type": "application/json",
        },
      },
    )
